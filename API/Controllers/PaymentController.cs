...
public class PaymentsController(PaymentServices paymentService,
    StoreContext context, IConfiguration config, ILogger<PaymentsController> logger) : BaseApiController
{
    ...
    public async Task<IActionResult> StripeWebhook()
    {
        ...
        try
        {
            ...
            else await HandlePymentIntentFailed(intent);

            return Ok();
        }
        catch (StripeException ex)
        {
            logger.LogError(ex, "Striipe webhook error");
    ...
    private async Task HandlePymentIntentFailed(PaymentIntent intent)
    {
